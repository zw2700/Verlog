# Category Metrics Summary

- episode_log: `logs/episode_log_rollout_cmu-gateway_us.anthropic.claude-haiku-4-5-20251001-v1_0_14263_no_all_voted_termination.jsonl`
- global_step range: `None` to `None`
- episodes analyzed: `100`
- overall consensus rate: `0.990`
- overall SO rate: `0.720`
- overall SO given consensus rate: `0.727`

| Category         | Episodes |     % | Consensus |    SO | SO given cons | Avg eff | Avg rank | Avg tokens | Avg turns |
| ---------------- | --------: | -----: | ---------: | -----: | -------------: | -------: | --------: | ----------: | ---------: |
| negotiation      |       72 | 0.720 |     0.986 | 0.667 |         0.676 |   0.958 |    1.507 |      164.0 |       6.5 |
| instant_decision |       28 | 0.280 |     1.000 | 0.857 |         0.857 |   0.992 |    1.143 |       29.9 |       2.0 |
